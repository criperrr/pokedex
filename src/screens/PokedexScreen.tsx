...
import { Result, PokemonData } from "../types/pokemon";
...
export default function PokedexScreen() {
  ...
  const [nomePokemon, setNomePokemon] = useState<string>("");
  const [pokemonBuscado, setPokemonBuscado] = useState<PokemonData | null>(
    null,
  );
  const [andGate, setAndGate] = useState<boolean>(false)
  const [mostrarDetalhes, setMostrarDetalhes] = useState<boolean>(false);
  const [carregando, setCarregando] = useState<boolean>(false);
  const [criterios, setCriterios] = useState<CriterioBusca[]>([
    { id: "1", campo: "nome", valor: "" },
  ]);

  const [regiaoSelecionada, setRegiaoSelecionada] = useState<string>("Todas");
  const [tipoSelecionado, setTipoSelecionado] = useState<string>("Todos");
  const [apenasLendarios, setApenasLendarios] = useState<boolean>(false);

  const [regioes, setRegioes] = useState<Result[]>([]);
  const [tipos, setTipos] = useState<Result[]>([]);

  const [todosPokemonNames, setTodosPokemonNames] = useState<string[]>([]);
  const [todasHabilidades, setTodasHabilidades] = useState<string[]>([]);
  const [todosMovimentos, setTodosMovimentos] = useState<string[]>([]);
  ...
  const adicionarCriterio = () => {
    ...
    setCriterios([...criterios, { id: novoId, campo: "nome", valor: "" }]);
  };
  ...
  const atualizarCriterio = (
    ...
    setCriterios(
      criterios.map((c) => (c.id === id ? { ...c, [chave]: valor } : c)),
    );
  };

  const limparTodosFiltros = () => {
    setCriterios([{ id: "1", campo: "nome", valor: "" }]);
  ...
  const getSuggestions = (campo: string) => {
      ...
      <View style={styles.mainWrapper}>
        ...
        {!mostrarAvancado ? (
          ...
          >
            ...
            <View style={[styles.actionButtonsContainer, { marginTop: 15 }]}>
              <Pressable
                style={[
                  styles.searchButton,
          ...
          <>
            ...
            <View style={styles.actionButtonsContainer}>
              <Pressable
                style={[styles.cleanButton, { borderColor: colors.border }]}
                onPress={limparTodosFiltros}
              >
                <Text style={[styles.cleanButtonText, { color: colors.text }]}>
                  Limpar filtros
                </Text>
              </Pressable>
