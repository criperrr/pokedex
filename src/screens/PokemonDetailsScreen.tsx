...
function getGenderLabel(rate: number) {
...
function getCatchRate(rate: number) {
...
export default function PokemonDetailsScreen({
  ...
  const [evolutionPaths, setEvolutionPaths] = useState<EvolutionStage[][]>([]);
    ...
    async function carregarPokemon() {
      try {
        ...
        if (typeof identifier === "number") {
          ...
          data = await getPokemonByName(identifier as string);
        }
        await load(data);
      } catch (e) {
  ...
  const getStatBarWidth = (baseStat: number): DimensionValue =>
    `${Math.min((baseStat / 255) * 100, 100)}%`;
      ...
      <View style={styles.mainWrapper}>
        ...
        <View style={styles.header}>
          <View style={{ flex: 1 }}>
            ...
            {species?.genus ? (
              <Text style={[styles.genus, { color: accentColor }]}>
                {species.genus}
              </Text>
            ) : null}
            {badges.length > 0 && (
        ...
        {species?.flavor_text ? (
          <SectionCard>
            <Text style={[styles.flavorText, { color: colors.text }]}>
              {`"${species.flavor_text}"`}
            </Text>
          </SectionCard>
        ) : null}
        ...
        <SectionCard>
          ...
          {species?.generation ? (
            <InfoRow
              label="Geração"
              value={formatGeneration(species.generation)}
            />
          ) : null}
          {species?.habitat ? (
            <InfoRow label="Habitat" value={capitalize(species.habitat)} />
          ) : null}
          {species?.color ? (
            <InfoRow label="Cor" value={capitalize(species.color)} />
          ) : null}
          {species?.shape ? (
            <InfoRow label="Forma" value={formatName(species.shape)} />
          ) : null}
          {species?.growth_rate ? (
            <InfoRow
              label="Taxa de crescimento"
              value={formatName(species.growth_rate)}
            />
          ) : null}
          {species?.hatch_counter != null ? (
            <InfoRow
              label="Ciclos de ovo"
              value={`${species.hatch_counter} ciclos (~${(species.hatch_counter + 1) * 255} passos)`}
            />
          ) : null}
          {species?.forms_switchable ? (
            <InfoRow label="Formas alteráveis" value="Sim" />
          ) : null}
        </SectionCard>
          ...
          {species != null && (
            <>
              <InfoRow
                ...
                value={getCatchRate(species.capture_rate)}
              />
              <InfoRow label="Felicidade base" value={species.base_happiness} />
              <InfoRow
                label="Gênero"
                value={getGenderLabel(species.gender_rate)}
              />
              {species.egg_groups.length > 0 && (
                <InfoRow
                  label="Grupos de ovo"
                  value={species.egg_groups.map(formatName).join(" · ")}
                />
              )}
            </>
          )}
          ...
          <InfoRow
            ...
            value={
              pokemon.stats
                ...
                .map(
                  (s) =>
                    `${s.effort} ${s.stat.name.toUpperCase().replace(/-/g, " ")}`,
                )
                .join(" · ") || "—"
            }
          />
          ...
          {pokemon.abilities.map((a) => (
            ...
            >
              ...
              {a.is_hidden && (
                ...
                >
                  <Text style={[styles.hiddenTagText, { color: accentColor }]}>
                    Oculta
                  </Text>
                </View>
              )}
        ...
        <SectionCard>
          ...
          >
            <Text style={[styles.statName, { color: colors.text }]}>TOTAL</Text>
            <Text
              style={[
                styles.statValue,
